...
| CSI-CHAIN-PUBLIC-01 | dated public-network case study (Ethereum Sepolia, ZKsync Era Sepolia) | ready | no | No source yet: no public transaction has been sent. Protocol `protocols/chain/CHAIN-PUBLIC-PROTOCOL-v1.md`, frozen by the baseline tag `chain-public-baseline-20260926`; harness commit `ed33276` (measurement code `8674988`); frozen public image `sha256:246e35a0…` (`chainbench/adapters/public/ARCHIVE.json`); frozen inputs `campaigns/chain/CSI-CHAIN-PUBLIC-01/inputs/`; session times, frozen endpoints, P and signer `0x6C58…c2E3` recorded; container dry run 92/92, pre-freeze checks and the funded read-only doctor (`readiness/`); results `results/chain-public-20260926/` (after the sessions). Reviewer entry point: `chainbench/adapters/public/README.md`. |
...
**Public-network case study, CSI-CHAIN-PUBLIC-01 (ready; baseline `chain-public-baseline-20260926`).** A separate, small, dated case study on Ethereum Sepolia and
...
`dry-run-public`, `check-public-inputs`, `test-guards-public`, `derive-public`; live commands for the author only, and
only in the frozen public image). Nothing has been sent to a public network yet.
