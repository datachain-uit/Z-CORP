# Z-CORP documentation

This directory contains the detailed setup, reproduction, and reference
documentation for Z-CORP.

## Setup

- [Setup overview](setup/README.md)
- [Docker environment](setup/docker.md)
- [Local Hardhat network](setup/local-hardhat.md)
- [Artifact-generation pipeline](setup/artifact-generation.md)

## Experiments

- [Experiment overview](experiments/README.md)
- [Blockchain deployment and verification](experiments/experiment-1-blockchain.md)
- [Circuit-growth measurement](experiments/experiment-2.1-constraints.md)
- [Groth16 prover-side performance](experiments/experiment-2.2-groth16.md)
- [PLONK prover-side performance](experiments/experiment-2.3-plonk.md)

## Reference

- [Repository layout](reference/repository-layout.md)
- [Scripts reference](reference/scripts.md)
- [Troubleshooting](reference/troubleshooting.md)

Return to the [repository README](../README.md).
