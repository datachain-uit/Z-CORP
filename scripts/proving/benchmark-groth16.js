...
function buildResultsPath(depths, index) {
    ...
    const filename = `${formatTimestamp()}-depth${minDepth}-${maxDepth}-groth16-index${index}.csv`;
...
function resolveWasmPath(depth) {
    ...
    if (!found) {
        throw new Error(
            `Missing wasm for depth ${depth}. Run compile_depth_circuits.js or setup_groth16_depth.js first.\n` +
...
async function exportVKeyIfNeeded(zkeyPath, vkeyPath) {
    if (fs.existsSync(vkeyPath)) {
        return JSON.parse(fs.readFileSync(vkeyPath, 'utf8'));
    }

    console.log(`  Exporting verification key from ${path.basename(zkeyPath)}...`);
...
async function benchmarkDepth(depth, index) {
    ...
    const zkeyPath = zkeyDepthFile(depth);
    const vkeyPath = vkeyDepthFile(depth);
    ...
    fs.mkdirSync(groth16WitnessDir, { recursive: true });
    fs.mkdirSync(groth16PublicProofDir, { recursive: true });
    fs.mkdirSync(groth16VkeysDir, { recursive: true });

    ensureFileExists(zkeyPath, `Missing zkey for depth ${depth}`);
    ...
    try {
        ...
        const vkey = await exportVKeyIfNeeded(zkeyPath, vkeyPath);
        const verifyStart = performance.now();
        const valid = await snarkjs.groth16.verify(vkey, publicSignals, proof);
        row.verify_s = ((performance.now() - verifyStart) / 1000).toFixed(3);

        if (!valid) {
            throw new Error('Groth16 verification failed');
        }
...
async function main() {
    ...
    console.log('=== Groth16 proving benchmark ===');
