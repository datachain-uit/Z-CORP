...
async function main() {
  ...
  let rpc;
  try { rpc = new Rpc(url, { label: process.env.CHAINBENCH_PUBLIC_RPC_LABEL_ERA_SEPOLIA || '', mode, timeoutMs: B.timing.rpc_timeout_ms, publicUrls: Object.values(B.endpoints.public_defaults) }); } catch (e) { if (e instanceof Refusal) die(e.code, e.message); throw e; }
  const started = now();
  ...
  const rec = { kind: 'era-batch-lifecycle', collection_id: id, mode, endpoint: rpc.identity, started_utc: started.utc, finished_utc: finished.utc, runs: runs.map((d) => path.relative(REPO, d)),
