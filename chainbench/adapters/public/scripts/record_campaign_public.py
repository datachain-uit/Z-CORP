...
verify, and the engineering dry run recorded under --dry-run passed every expectation. The ready and scientific stages
(after the author inputs and the baseline tag) are added to this tool when they are reached."""
...
rec = {
    ...
    'harness': {'commit': hc, 'adapter': 'chainbench/adapters/public', 'procedure': 'chainbench/workloads/zcorp/' + B['procedure'].split('zcorp/')[-1],
    ...
    'author_inputs_open': ['session start times (S1-S3, at least two UTC days)', 'Sepolia endpoint and provider label', 'Era Sepolia endpoint and provider label',
                           'dedicated signing key and its address', 'funding on both networks', 'Sepolia priority fee P (default 1 gwei)', 'results directory date label',
                           'containerized dry-run-public and live read-only doctor-public on the campaign host; public baseline tag'],
